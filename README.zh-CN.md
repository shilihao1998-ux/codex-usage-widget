...
- **预设主题**：Midnight / Paper / Terminal / Nord，作为起点；主题可导出/导入为 JSON 分享
- **背景图**：设置里直接点选自带的三张壁纸，或选任意本地图片（png/jpg/gif/webp/bmp/avif，≤12 MB）；fit 支持 cover / contain / stretch / tile
...
设置 → **Behavior**。这里的每一项都是可选的；**唯一永远在的是剩余额度那两行**。

| 设置 | 默认 | 作用 |
| --- | --- | --- |
| 紧凑模式 | 关 | 去掉倒计时行 |
| 显示全部计量桶 | 关 | 例如 Spark |
| 显示 credits | 开 | 账号没有 credits 时**一个像素都不画** |
| **燃烧速率 + 预计耗尽** | **关** | 见下 |
| 托盘图标 | 环 | 环 · 环+数字 · 只显示数字 |
| 托盘跟随 | 较低的那个窗口 | 也可固定看 5h 或周窗口 |
| 低额度提醒 + 阈值 | 开，20% / 10% | 每个窗口每个阈值只提醒一次 |
| 额度恢复时告诉我 | 开 | 只对**确实警告过**的窗口触发 |
| 额外桶也提醒 | 关 | Spark 之类默认不吵你 |
| 轮询间隔 | 1 分钟 | 15 秒 … 15 分钟（服务端推送仍会立刻刷新） |

**燃烧速率是全产品唯一的估算值，默认关闭。** 打开后每个窗口多一条灰色小字，例如 `~9.1%/h · runs out ~15:40 · est.`。它来自本组件自己的历史、不是 Codex 给的，所以：

- 样本不足（<3 个 / 跨度 <10 分钟 / 变化 <2%）时显示 `measuring…`，**拒绝编一个数字**；
- 绝不跨窗口重置去拟合；
- **绝不染环形图、绝不染托盘、绝不发通知**。

### Token 用量 & 额度趋势（都默认关）

![全部打开的样子](docs/widget-full.png)

| 面板 | 数据来源 | 默认 |
| --- | --- | --- |
| **Tokens** | `account/usage/read`——官方每日 token 用量、连续使用天数、累计总量。**是实测值，不是估算。** 需要较新的 Codex；旧版本会直接说明不支持，而不是画一张空图。 | 关 |
| **额度趋势** | 本组件自己的历史。x 轴是真实时间，每次重置断线，组件没运行的空档画成**缺口**——插值等于凭空画出没人测过的那几个小时。 | 关 |

### 窗口

设置 → **Behavior** → Window：锁定位置、点击穿透（鼠标直接穿过卡片）、鼠标移开时淡出、全局热键显示/隐藏。热键被别的程序占用时会明确报错，而不是静默失效。

### 语言

设置 → **Behavior** → Language：English / 简体中文。**只翻译界面**——Codex 返回的内容（plan 名、limit id）和所有面向机器的接口（CLI、HTTP API、历史文件）保持原样，脚本不用关心你的语言。

### 数据

设置 → **Data**：停止记录历史、清除历史、导出（JSON / CSV）、打开数据目录、开关更新检查。历史会自动瘦身：一周前的行降到每小时一条，但**只删行、不合成行**——把两个样本平均一下就等于造出服务端从未返回过的百分比；每个重置边界和峰值都会保留。
...
- 自带两个示例（都默认关闭）：`plugins/weather/`（Open-Meteo，免 API key）和 `plugins/openai-status/`（**只在 OpenAI 真的出事故时才渲染**——一次失败的 turn，更多时候是官方挂了而不是你额度耗尽）
...
node bin/codex-usage.js once --json   # JSON 输出（带 "schema": "codex-usage.v1"）
node bin/codex-usage.js statusline    # 一行输出，给 shell 提示符 / 状态栏用
node bin/codex-usage.js watch         # 常驻，变化即打印
node bin/codex-usage.js verify        # 实时读数 vs Codex 自己记的快照
node bin/codex-usage.js report --open # 可分享的 HTML 报告（不含任何账号信息）
node bin/codex-usage.js doctor        # 诊断（路径脱敏、邮箱打码，可直接贴 issue）
node bin/codex-usage.js mcp           # MCP server：让 agent 自己查配额
node bin/codex-usage.js serve --port 7893
```

**`mcp` 是别人都没有的那个集成**：把它接到 Codex 上，agent 在开始大重构**之前**就能自己问"我还剩多少额度"。它只返回实测值和数据年龄，**绝不返回预测**——一个估算值塞进模型上下文，模型会把它当事实复述出来。见 [MCP.md](MCP.md)。

**`doctor`** 把排障章节的每一条都变成两秒钟的机器检查：选中了哪个 codex 二进制、是否已登录、两个 app-server 方法是否都能应答、Codex 自己在 stderr 上打印了什么。路径缩成 `~`、邮箱打码，输出可以直接贴到 issue 里。

`statusline` 输出 `codex 88% 5h (2h 17m) · 78% wk`，约 60 毫秒——它读缓存快照，**不会去拉起 app-server**（那要两秒，提示符会卡），并且从绝对重置时间重算倒计时，所以缓存文件永远不会显示一个冻住的倒计时。

`once` 和 `statusline` 还有**退出码契约**，git hook / 脚本可以据此拒绝启动昂贵任务：
...
- **没有任何遥测。** 应用自己只发一个请求：每天一次、不带认证的 `GET` 打 GitHub releases API，看有没有新版本。不带任何标识、不下载、不安装，设置 → Data 里可以关掉。它存在的理由是：本组件读的是一个**未文档化**的 Codex 私有接口，OpenAI 改了它，冻结的旧版本可能会安静地显示错数字。
- 本地状态在 `~/.codex-usage-widget/`（可用 `CODEX_USAGE_DATA_DIR` 改）：`prefs.json`、`state.json`（最近快照，**含账号邮箱和 plan**）、`history.jsonl`（额度变化历史）、以及你的插件。不会写进仓库。设置 → **Data** 可以停止记录、清除、导出（JSON / CSV）。
- 认证完全是 Codex 的事：本项目不读 `~/.codex/auth.json`，也不碰任何 token，只是问已经登录好的本机 app-server。
- 启用的插件会自己发网络请求——自带的天气插件调 [Open-Meteo](https://open-meteo.com/)（免费、无需 key，CC-BY 4.0 署名）。**你自己丢进插件目录的插件默认是关的**，必须你手动启用才会跑。
