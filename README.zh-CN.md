...
目前没有打包好的安装包，需要从源码跑（要有 Node 和终端）。
...
- **背景图**：任意本地图片（png/jpg/gif/webp/bmp/avif，≤12 MB），fit 支持 cover / contain / stretch / tile；自带三张壁纸在 `assets/backgrounds/`（`npm run backgrounds` 可重新生成）
